...
We need to implement rate limiting in a distributed environment to prevent abuse and ensure fair usage of our API. The
algorithm should support burst traffic, be easy to reason about, and allow flexibility in configuration.
...
- If not implemented carefully, concurrency issues may occur — we'll mitigate this by using Lua scripts in Redis for
  atomicity.
...
This ADR will be revisited if performance issues arise under extreme load or if business requirements change (e.g.,
needing dynamic policy updates or adaptive throttling).
